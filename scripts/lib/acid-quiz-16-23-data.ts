/**
 * 酸鹼單元第 16～23 部影片小考題（每部 3 題）— 與 patch-acid-quiz-16-23 共用。
 * restore-acid-video-quizzes 依 sort_order 套用，不依賴舊的 quiz_questions.id。
 */

export type AcidQuiz16_23Patch = {
...
const CLEAR_IMAGES = {
  ...
  choice_a_image_url: null as string | null,
  choice_b_image_url: null as string | null,
  choice_c_image_url: null as string | null,
  choice_d_image_url: null as string | null,
};

/** 供依舊 UUID 更新後台題目列時使用（patch-acid-quiz-16-23.ts） */
export const ACID_QUIZ_16_23_UPDATES: Record<string, AcidQuiz16_23Patch> = {
...
/** patch 檔內題目順序：每 3 題對應 sort_order 16、17、…、23 */
export const ACID_QUIZ_16_23_UUID_ORDER = [
  "bbc452ec-020d-41bb-8fde-86532224bec8",
  "f23f3bf4-28a5-45d8-83f2-7aa7195c13ac",
  "62536bbd-fca4-409d-baa0-3ddb05716b75",
  "5eba957a-b064-4ba5-9b35-65947be793a7",
...
export function acidPatchesForVideoSortOrder(sortOrder: number): AcidQuiz16_23Patch[] | null {
  if (sortOrder < 16 || sortOrder > 23) return null;
  const start = (sortOrder - 16) * 3;
  const ids = ACID_QUIZ_16_23_UUID_ORDER.slice(start, start + 3);
  const u = ACID_QUIZ_16_23_UPDATES;
  return ids.map((id) => {
    const p = u[id];
    if (!p) throw new Error(`acid-quiz-16-23-data: 缺少 id ${id}`);
    return p;
  });
}
