...
export default async function LearningSetupHelpPage() {
  ...
  return (
    <>
      ...
      <ol className="list-decimal space-y-4 rounded-2xl border border-cyan-200/50 bg-white shadow-[0_8px_28px_-10px_rgba(14,165,233,0.12)] p-6 pl-10 text-sm leading-relaxed text-slate-600 shadow-sm">
        <li>
      ...
      <details className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-4 text-sm text-slate-400 shadow-sm">
        <summary className="cursor-pointer select-none font-medium text-slate-700">
          技術人員參考（選讀）
        </summary>
        <p className="mt-3 text-xs leading-relaxed">
          需存在 learning_tasks 等資料表；若 initial migration 中的共用函式尚未建立，請一併套用。詳細檔名與路徑請見專案{" "}
          <code className="rounded bg-slate-100 px-1 py-0.5 text-[11px] text-slate-700">supabase/migrations/</code> 目錄。
        </p>
      </details>
      ...
      <div className="flex flex-wrap gap-3">
        <Link
          href="/admin"
          className="interactive-btn inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-200/90 bg-cyan-50/40 px-5 py-2.5 text-sm font-medium text-slate-700 shadow-sm hover:bg-cyan-50/80"
        >
          返回後台首頁
        </Link>
        <Link
