  ...
  if (!examScopeId) {
    return (
      <div className="rounded-2xl border border-amber-200/90 bg-amber-50/90 p-6 text-amber-950 shadow-md">
        ...
        <p className="mt-2 text-sm text-slate-600">請設定 NEXT_PUBLIC_DEFAULT_EXAM_SCOPE_ID 或於資料庫建立 scope。</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-400">
          ...
          <strong className="text-slate-700">JSON 批次編輯</strong>
          ，可增減題數）。儲存後學生端立即套用。
        </p>
        {scopeTitle ? (
          <p className="mt-2 text-xs text-slate-500">
            段考範圍：<span className="font-medium text-slate-600">{scopeTitle}</span>
          </p>
        ) : null}
      </div>
      ...
      <div className="space-y-10">
        {rows.map((block) => (
          <section key={block.unitId} className="rounded-2xl border border-cyan-200/50 bg-white shadow-[0_8px_28px_-10px_rgba(14,165,233,0.12)] shadow-[0_8px_36px_-16px_rgba(0,0,0,0.45)]">
            <h2 className="border-b border-slate-200 px-4 py-3 text-lg font-semibold text-slate-900 sm:px-5">
              {block.unitTitle}
            </h2>
