  ...
  if (!detail) {
    return (
      <p className="text-slate-600">
        找不到學生資料。{" "}
        <Link href="/admin/video-tracking" className="font-medium text-cyan-700 underline">
          返回總覽
        </Link>
      </p>
    );
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-lg font-medium text-slate-700">{st.name}</p>
        {st.class_name ? (
          <p className="text-sm text-slate-400">{st.class_name} 班</p>
        ) : null}
      </div>

      <section className="rounded-2xl border border-cyan-200/50 bg-white shadow-[0_8px_28px_-10px_rgba(14,165,233,0.12)] p-5 shadow-md sm:p-6">
        ...
        <ul className="mt-4 space-y-3 text-sm">
          {(detail.progress as { video_id: string; completion_rate: number; is_completed: boolean }[]).map(
            (p) => (
              <li
                key={p.video_id}
                className="flex flex-wrap items-baseline justify-between gap-2 rounded-xl bg-slate-50/80 px-3 py-2.5 ring-1 ring-slate-200/90"
              >
                <span className="font-medium text-slate-900">{videoMap.get(p.video_id) ?? "影片"}</span>
                <span className="text-slate-600">
                  觀看進度 {Number(p.completion_rate).toFixed(0)}%
                  {p.is_completed ? (
                    <span className="ml-2 text-cyan-800">· 已完成</span>
                  ) : (
        ...
        <ul className="mt-4 space-y-3 text-sm">
          {(detail.attempts as { quiz_id: string; score: number; is_passed: boolean }[]).length === 0 ? (
            ...
            (detail.attempts as { quiz_id: string; score: number; is_passed: boolean }[]).map((a, i) => (
              <li
                key={i}
                className="rounded-xl bg-slate-50/80 px-3 py-2.5 text-slate-700 ring-1 ring-slate-200/90"
              >
                得分 {a.score} 分 ·{" "}
                {a.is_passed ? (
                  <span className="font-medium text-cyan-800">已通過</span>
                ) : (
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-slate-50/70 p-5 shadow-sm sm:p-6">
