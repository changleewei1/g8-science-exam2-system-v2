...
export default async function AdminTaskDetailPage({
  ...
  try {
    ...
    return (
      <div className="rounded-2xl border border-red-200/90 bg-red-50/90 p-6 text-red-900 shadow-md">
        ...
        <details className="mt-4 rounded-lg border border-rose-200 bg-rose-50 p-3 text-xs text-rose-900">
  ...
  if (!detail) {
    return (
      <div className="rounded-2xl border border-cyan-200/50 bg-white p-6 text-slate-600 shadow-[0_8px_28px_-10px_rgba(14,165,233,0.12)]">
        <p>找不到此學習任務。</p>
        <Link
          href="/admin/tasks"
          className="interactive-nav mt-3 inline-block text-sm font-medium text-cyan-700 underline-offset-4 hover:underline"
        >
          返回學習任務設定
        </Link>
      </div>
  ...
  return (
    <div className="space-y-10">
      {sp.created === "1" ? (
        <div className="rounded-xl border border-cyan-400/35 bg-cyan-500/10/90 px-4 py-3 text-sm text-cyan-900">
          已成功建立學習任務
        </div>
      ) : null}
      ...
      <div>
        ...
        <dl className="mt-4 grid gap-2 text-sm text-slate-600 sm:grid-cols-2">
          ...
          <div>
            ...
            <dd className="font-semibold text-cyan-900">{overall}%</dd>
          </div>
        </dl>
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <Link
            ...
            className="inline-flex min-h-10 items-center rounded-xl border border-slate-200/90 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm"
          >
            編輯任務
          </Link>
        </div>
      ...
      <section className="space-y-3">
        ...
        <div className="overflow-x-auto rounded-2xl border border-cyan-200/50 bg-white shadow-[0_8px_28px_-10px_rgba(14,165,233,0.12)]">
          <table className="min-w-[960px] w-full text-left text-sm">
            ...
            <tbody>
              {videos.map((v) => (
                <tr key={`${v.videoId}-${v.dayIndex}`} className="border-t border-slate-200">
                  <td className="px-4 py-3 text-slate-700">第 {v.dayIndex} 天</td>
                  <td className="px-4 py-3 text-slate-600">{v.unitTitle}</td>
          ...
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {students.length === 0 ? (
                ...
                students.map((s) => (
                  <tr key={s.studentId} className="border-t border-slate-200">
                    <td className="px-4 py-3 font-mono text-slate-600">{s.studentCode}</td>
                    <td className="px-4 py-3 font-medium text-slate-900">{s.name}</td>
                    <td className="px-4 py-3 text-slate-600">
                      {s.completedCount}/{s.totalVideos}
                    </td>
                    <td className="px-4 py-3 text-slate-700">{s.completionRate}%</td>
                  </tr>
                ))
              )}
            </tbody>
