...
export default async function AdminTaskDetailPage({
  ...
  try {
    ...
    return (
      <div className="rounded-2xl border border-red-200/90 bg-red-50/90 p-6 text-red-900 shadow-md">
        ...
        <details className="mt-4 rounded-lg border border-rose-400/40 bg-rose-950/35 p-3 text-xs">
  ...
  if (!detail) {
    return (
      <div className="rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-md p-6 text-slate-300 shadow-md">
        <p>找不到此學習任務。</p>
        <Link
          href="/admin/tasks"
          className="interactive-nav mt-3 inline-block text-sm font-medium text-cyan-300 underline-offset-4 hover:underline"
        >
          返回學習任務設定
        </Link>
      </div>
  ...
  return (
    <div className="space-y-10">
      {sp.created === "1" ? (
        <div className="rounded-xl border border-cyan-400/35 bg-cyan-500/10/90 px-4 py-3 text-sm text-cyan-100">
          已成功建立學習任務
        </div>
      ) : null}
      ...
      <div>
        ...
        <h1 className="mt-3 text-2xl font-semibold text-slate-50">任務內容</h1>
        <p className="mt-2 text-xl font-semibold text-slate-50">{task.title}</p>
        {task.description ? (
        ...
        <dl className="mt-4 grid gap-2 text-sm text-slate-300 sm:grid-cols-2">
          <div>
            ...
            <dd className="font-medium text-slate-50">{assigneeText}</dd>
          </div>
          <div>
          ...
          <div>
            ...
            <dd className="font-semibold text-cyan-100">{overall}%</dd>
          </div>
        </dl>
        <div className="mt-6 flex flex-wrap gap-3">
          <Link
            href={`/admin/tasks/${taskId}/progress`}
            className="inline-flex min-h-10 items-center rounded-xl bg-cyan-600 px-4 py-2 text-sm font-medium text-white shadow-md"
          >
            查看進度
          </Link>
          <Link
            href={`/admin/tasks/${taskId}/edit`}
            className="inline-flex min-h-10 items-center rounded-xl border border-white/15 bg-white px-4 py-2 text-sm font-medium text-slate-200 shadow-sm"
          >
            編輯任務
          </Link>
        </div>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-slate-50">指定影片清單</h2>
        <p className="text-sm text-slate-400">含單元、技能代碼與是否已設定測驗</p>
        <div className="overflow-x-auto rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-md shadow-[0_8px_36px_-16px_rgba(0,0,0,0.45)]">
          <table className="min-w-[960px] w-full text-left text-sm">
            ...
            <tbody>
              {videos.map((v) => (
                <tr key={`${v.videoId}-${v.dayIndex}`} className="border-t border-white/10">
                  <td className="px-4 py-3 text-slate-200">第 {v.dayIndex} 天</td>
                  <td className="px-4 py-3 text-slate-300">{v.unitTitle}</td>
                  <td className="px-4 py-3 font-medium text-slate-50">{v.title}</td>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-semibold text-slate-50">學生完成概況</h2>
        <p className="text-sm text-slate-400">詳細測驗與時間請至「查看進度」或個別學習報告</p>
        <div className="overflow-x-auto rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-md shadow-[0_8px_36px_-16px_rgba(0,0,0,0.45)]">
              ...
              {students.length === 0 ? (
                ...
                students.map((s) => (
                  <tr key={s.studentId} className="border-t border-white/10">
                    <td className="px-4 py-3 font-mono text-slate-300">{s.studentCode}</td>
                    <td className="px-4 py-3 font-medium text-slate-50">{s.name}</td>
                    <td className="px-4 py-3 text-slate-300">
                      {s.completedCount}/{s.totalVideos}
                    </td>
                    <td className="px-4 py-3 text-slate-200">{s.completionRate}%</td>
                  </tr>
                ))
              )}
            </tbody>
