...
export function TaskCreateForm({ videos, editTaskId }: Props) {
  ...
  if (loadEdit) {
    return (
      <div className="rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-md p-8 text-center text-slate-400 shadow-md">
        載入任務資料…
      </div>
    );
  }
  ...
  if (videos.length === 0) {
    return (
      <div className="rounded-2xl border border-amber-200/90 bg-amber-50/90 p-5 text-sm text-amber-950 shadow-sm">
        <p className="font-medium text-slate-50">資料庫中尚無影片資料</p>
        ...
        <ul className="mt-3 list-inside list-disc space-y-1 text-slate-200">
          <li>
            <Link
              ...
              className="font-medium text-cyan-200 underline underline-offset-2"
            >
              學習系統與匯入設定說明
            </Link>
          </li>
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="space-y-6 rounded-2xl border border-white/10 bg-white/[0.06] backdrop-blur-md p-6 shadow-md sm:p-8"
    >
      <div>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <label className="block text-sm sm:col-span-2">
          <span className="font-medium text-slate-200">任務名稱</span>
          <input
            required
            className="mt-1.5 w-full rounded-xl border border-white/15 px-3 py-2.5 text-slate-50 shadow-sm"
        ...
        <label className="block text-sm sm:col-span-2">
          <span className="font-medium text-slate-200">任務說明</span>
          <textarea
            className="mt-1.5 w-full rounded-xl border border-white/15 px-3 py-2.5 text-slate-50 shadow-sm"
            rows={2}
            value={description}
      ...
      <fieldset className="space-y-3 rounded-xl border border-white/10 bg-white/[0.04] p-4">
        <legend className="text-sm font-medium text-slate-200">指派對象</legend>
        ...
        {assignmentMode === "class" ? (
          <label className="block text-sm">
            <span className="text-slate-300">班級</span>
            <input
              required
              className="mt-1.5 w-full max-w-md rounded-xl border border-white/15 px-3 py-2.5 shadow-sm"
              placeholder="例如：801"
              value={className}
          ...
          <div className="max-h-48 overflow-y-auto rounded-xl border border-white/10 bg-white/[0.06] backdrop-blur-md p-3 text-sm">
      ...
      <div>
        <div className="mb-1">
          <span className="font-medium text-slate-200">完成期限</span>
        ...
        <div className="mt-2 grid gap-4 sm:grid-cols-2">
          <label className="block text-sm">
            ...
            <input
              required
              type="date"
              className="mt-1.5 w-full rounded-xl border border-white/15 px-3 py-2.5 shadow-sm"
              value={startDate}
              onChange={(e) => {
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4">
        <div className="mb-2">
          <span className="font-medium text-slate-200">學習內容（影片）</span>
        ...
        <div className="mb-3 flex items-center justify-end">
          <button
            type="button"
            onClick={addRow}
            className="interactive-nav rounded-lg px-2 py-1 text-sm font-medium text-cyan-300 underline decoration-teal-700/40 underline-offset-2"
          >
            新增影片
          </button>
        </div>
        <ul className="space-y-3">
          {rows.map((row, i) => (
            <li
              key={i}
              className="flex flex-wrap items-end gap-2 rounded-lg bg-white/[0.12] p-3 ring-1 ring-white/10/80"
            >
              ...
              <label className="w-32 text-sm">
                ...
                <input
                  ...
                  type="number"
                  min={1}
                  className="mt-1 w-full rounded-lg border border-white/15 px-2 py-2"
                  value={row.dayIndex}
      ...
      <div className="flex flex-wrap gap-3 pt-2">
        ...
        <button
          ...
          className="interactive-btn rounded-xl border border-white/15 bg-white px-6 py-2.5 text-sm font-medium text-slate-200 shadow-sm"
        >
          取消
        </button>
        <button
          ...
          className="interactive-btn rounded-xl border border-white/10 bg-white/[0.04] px-6 py-2.5 text-sm font-medium text-slate-300 shadow-sm"
        >
          返回任務列表
        </button>
      </div>
