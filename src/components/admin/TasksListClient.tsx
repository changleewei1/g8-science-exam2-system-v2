...
export function TasksListClient({ tasks }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 rounded-2xl border border-cyan-200/50 bg-white shadow-[0_8px_28px_-10px_rgba(14,165,233,0.12)] p-4 shadow-sm sm:flex-row sm:flex-wrap sm:items-end">
        <label className="block min-w-[140px] flex-1 text-sm">
          <span className="font-medium text-slate-700">搜尋任務</span>
          <input
            type="search"
            className="mt-1.5 w-full rounded-xl border border-slate-200/90 px-3 py-2 text-sm"
        ...
        <label className="block min-w-[160px] text-sm">
          <span className="font-medium text-slate-700">任務狀態</span>
          <select
            className="mt-1.5 w-full rounded-xl border border-slate-200/90 px-3 py-2 text-sm"
            value={phaseFilter}
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-10 text-center text-slate-400">
                  {tasks.length === 0 ? (
                    <>
                      <p className="font-medium text-slate-700">目前尚未建立學習任務</p>
              ...
              filtered.map((t) => (
                <tr key={t.id} className="border-t border-slate-200">
                  ...
                  <td className="px-4 py-3 text-slate-600">{t.assigneeDisplay}</td>
                  <td className="px-4 py-3 text-slate-600">{t.videoCount}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-slate-600">{t.startDate}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-slate-600">{t.endDate}</td>
                  <td className="px-4 py-3 text-slate-700">{t.aggregateCompletionRate}%</td>
