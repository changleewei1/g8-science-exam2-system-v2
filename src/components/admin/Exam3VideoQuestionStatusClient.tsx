...
function rowClass(r: Readiness): string {
  if (r === "ready") return "border-b border-slate-200 bg-emerald-50/60";
  if (r === "pending_review") return "border-b border-slate-200 bg-amber-50/50";
  return "border-b border-slate-200 bg-rose-50/40";
}
    ...
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <button
          type="button"
          onClick={() => void load()}
          className="interactive-btn rounded-lg border border-slate-200/90 bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm"
        >
          重新整理
        </button>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-cyan-200/50 bg-white shadow-[0_8px_28px_-10px_rgba(14,165,233,0.12)]">
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-slate-200 bg-slate-50/80 text-xs font-medium uppercase text-slate-400">
            <tr>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.videoId} className={rowClass(r.readiness)}>
                ...
                <td className="max-w-[120px] truncate px-3 py-2 text-slate-600">{r.unitTitle}</td>
