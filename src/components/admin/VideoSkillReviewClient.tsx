...
export function VideoSkillReviewClient() {
  ...
  if (loading) return <p className="text-sm text-slate-600">載入候選資料中…</p>;
  ...
  if (rows.length === 0) {
    return (
      <div className="space-y-3">
        ...
        <p className="rounded-xl border border-slate-200 bg-white p-5 text-sm text-slate-600">目前沒有待審核候選。</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {rows.map((r) => {
        ...
        return (
          <section key={r.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="text-right text-sm">
                ...
                <p className="text-slate-600">{r.suggested_skill_name || "-"}</p>
                <p className="text-slate-600">confidence: {typeof r.confidence === "number" ? r.confidence.toFixed(2) : "-"}</p>
            ...
            <p className="mt-3 rounded bg-slate-50 p-3 text-sm text-slate-700">{r.reason || "(無理由)"}</p>
